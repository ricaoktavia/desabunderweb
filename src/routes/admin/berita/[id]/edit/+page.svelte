<script lang="ts">
	...
	let previewUrl = $state<string | null>(data.berita.gambar);
...
<div class="rounded-2xl border border-gray-200 bg-white shadow-sm p-6 sm:p-8 max-w-3xl">
	<form action="?/update" method="POST" enctype="multipart/form-data" class="space-y-6" use:enhance={() => {
		loading = true;
		return async ({ update }) => {
			await update();
			loading = false;
		...
		<input type="hidden" name="currentGambar" value={data.berita.gambar || ''} />

		<div>
			<label for="judul" class="block text-sm font-semibold text-gray-900 mb-2">Judul Berita *</label>
			<input type="text" id="judul" name="judul" value={data.berita.judul} required class="block w-full rounded-xl border-gray-300 py-3 px-4 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50" />
		</div>
		...
		<div>
			...
			<textarea id="konten" name="konten" rows="12" required class="block w-full rounded-xl border-gray-300 py-3 px-4 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50">{data.berita.konten}</textarea>
		</div>
