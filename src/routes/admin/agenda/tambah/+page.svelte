...
<div class="rounded-2xl border border-gray-200 bg-white shadow-sm p-6 sm:p-8 max-w-3xl">
	<form method="POST" class="space-y-6" use:enhance={() => { loading = true; return async ({ update }) => { await update(); loading = false; }; }}>
		<div>
			<label for="judul" class="block text-sm font-semibold text-gray-900 mb-2">Judul Kegiatan *</label>
			<input type="text" id="judul" name="judul" required class="block w-full rounded-xl border-gray-300 py-3 px-4 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50" />
		</div>
		
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
			<div>
				<label for="tanggalKegiatan" class="block text-sm font-semibold text-gray-900 mb-2">Tanggal & Waktu *</label>
				<input type="datetime-local" id="tanggalKegiatan" name="tanggalKegiatan" required class="block w-full rounded-xl border-gray-300 py-3 px-4 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50" />
			</div>
			<div>
				<label for="lokasi" class="block text-sm font-semibold text-gray-900 mb-2">Lokasi *</label>
				<input type="text" id="lokasi" name="lokasi" required class="block w-full rounded-xl border-gray-300 py-3 px-4 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50" />
			</div>
		</div>

		<div>
			<label for="deskripsi" class="block text-sm font-semibold text-gray-900 mb-2">Deskripsi Kegiatan</label>
			<textarea id="deskripsi" name="deskripsi" rows="6" class="block w-full rounded-xl border-gray-300 py-3 px-4 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50"></textarea>
		</div>
