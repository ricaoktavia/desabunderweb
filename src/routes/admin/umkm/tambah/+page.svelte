...
<div class="mb-6 flex items-center justify-between">
	<div class="flex items-center gap-4">
		...
		<h1 class="text-2xl font-bold text-gray-900">Tambah UMKM</h1>
	</div>
</div>
	...
	<form method="POST" enctype="multipart/form-data" class="space-y-6" use:enhance={() => { loading = true; return async ({ update }) => { await update(); loading = false; }; }}>
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
			<div>
				...
				<input type="text" id="namaUsaha" name="namaUsaha" required class="block w-full rounded-xl border-gray-300 py-3 px-4 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50" />
			</div>
			<div>
				<label for="namaPemilik" class="block text-sm font-semibold text-gray-900 mb-2">Nama Pemilik *</label>
				<input type="text" id="namaPemilik" name="namaPemilik" required class="block w-full rounded-xl border-gray-300 py-3 px-4 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50" />
			</div>
		</div>
		...
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
			<div>
				...
				<input type="text" id="kontak" name="kontak" class="block w-full rounded-xl border-gray-300 py-3 px-4 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50" />
			</div>
			<div>
				<label for="alamat" class="block text-sm font-semibold text-gray-900 mb-2">Alamat Usaha</label>
				<input type="text" id="alamat" name="alamat" class="block w-full rounded-xl border-gray-300 py-3 px-4 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50" />
			</div>
		</div>

		<div>
			<label for="deskripsi" class="block text-sm font-semibold text-gray-900 mb-2">Deskripsi Produk/Usaha</label>
			<textarea id="deskripsi" name="deskripsi" rows="4" class="block w-full rounded-xl border-gray-300 py-3 px-4 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50"></textarea>
		</div>
		...
		<div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
			...
			<button type="submit" disabled={loading} class="inline-flex items-center gap-2 rounded-xl bg-blue-700 px-6 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-800 disabled:opacity-70">
				{#if loading}<Loader2 class="h-4 w-4 animate-spin" />{:else}<Save class="h-4 w-4" />{/if} Simpan
			</button>
		</div>
	</form>
</div>
